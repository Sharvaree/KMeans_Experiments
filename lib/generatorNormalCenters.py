...
def generatorNormCenters(n,d,k,rang,sds,z,num):
    #Constants
    ...
    for i in range(k):
        for j in range(s[i]):
            ...
            vec = np.array(point) - np.array(centers[i])
            pt = np.array(point)
            norm = np.linalg.norm(np.array(vec))
            if(norm > sds[0]):
                pt = np.array(centers[i]) - sds[0]*(vec/norm)
            point = list(pt)
    ...
    fn = "syntheticDataCenters/n" + str(len(pointset)) + "d" + str(d) + "k" + str(k) + "rang" + str(int(rang)) + "z" + str(z) +"c"+ str(num) + "s" + str(sds[0]) + ".csv"
